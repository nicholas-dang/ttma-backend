...
function edit(inputData, req, res){
            ...
            //Edit income
            sql = "UPDATE TaxDeductionsExpenses SET Type = (?), Amount = (?), Date = (?), Description = (?), YearID = (?) WHERE TaxID = (?);";
            connection.query(sql,
              ...
              (err, results) => {
                if (!err) {
                  console.log("Deduction/Expense Successfully Edited");
                  respond(res, 201, 
                    {
                      message: 'Deduction/Expense Successfully Edited', 
                      result: 0,
                    });
                  ...
                  console.error(err);
                  error(res, 500, "Failed to Edit Deduction/Expense");
                }
              }
            )
...
function editExpense(req, res) {
  if (req.method === "POST" && req.url === "/editExpense") {
    let data = "";
