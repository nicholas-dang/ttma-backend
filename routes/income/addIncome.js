...
function insert(UserID,inputData, req, res){
  //add to account
  sql = "INSERT INTO AdditionalIncome (UserID, Title, Amount, Date, Description, YearID) VALUES (?,?,?,?,?,?);" +
  ...
  connection.query(sql,
    ...
    (err, results) => {
      if (!err) {
          console.log("Income Added to Database");
          respond(res, 201, 
            {
              message:{
        ...
        console.error(err);
        error(res, 500, "Failed to add Income");
        respond(res, 500, 
          {
...
function InsertIncome(UserID,inputData, req, res) {
...
function addIncome(req, res) {
  if (req.method === "POST" && req.url === "/addIncome") {
    let data = "";
    ...
    req.on("end", () => {
      try {
        ...
        if (UserID == -1) {
          ...
          InsertIncome(UserID,inputData,req,res);
      } catch (error) {
        console.error(error);
